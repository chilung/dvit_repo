#!/usr/bin/env python
"""
DeepViT training and evaluating script
...
from timm.data import Dataset, create_loader, resolve_data_config, Mixup, FastCollateMixup, ImageFolderLMDB
...
import models
from loss import SoftTargetCrossEntropyCosReg

from distutils.dir_util import copy_tree

from utils.misc import bn_calibration
...
def main():
    ...
    if args.jsd:
        ...
        if args.cos_reg_component > 0:
            ...
            train_loss_fn = SoftTargetCrossEntropyCosReg(n_comn = args.cos_reg_component).cuda()
        else:
    ...
    try:
        for epoch in range(start_epoch, num_epochs):
            ...
            if args.max_iter > 0:
                _ = validate(model, loader_cali, validate_loss_fn, args, amp_autocast=amp_autocast, use_bn_calibration=True)
...
def train_epoch(
        ...
        loss_scaler=None, model_ema=None, mixup_fn=None, dis_fn = torch.nn.CosineSimilarity(dim=1)):
    ...
    for batch_idx, (input, target) in enumerate(loader):
        ...
        with amp_autocast():
            if 'vit_base_patch16_224_share_attn_cos_reg' in args.model or args.use_cos_reg_component:
                output, atten_list = model(input)
                loss = loss_fn(output, target, atten_list)
            else:
                output = model(input)
...
def validate(model, loader, loss_fn, args, amp_autocast=suppress, log_suffix='', cali_loader=None, use_bn_calibration=False):
    batch_time_m = AverageMeter()
    losses_m = AverageMeter()
    top1_m = AverageMeter()
    top5_m = AverageMeter()
    ...
    last_idx = len(loader) - 1
    if use_bn_calibration:
        model.apply(bn_calibration)
    with torch.no_grad():
        for batch_idx, (input, target) in enumerate(loader):
            last_batch = batch_idx == last_idx
            if use_bn_calibration and batch_idx >= args.max_iter:
                break
            if not args.prefetcher:
                input = input.cuda()
                target = target.cuda()
